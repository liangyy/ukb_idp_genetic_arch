...
fix_loading = function(res) {
  all_pos = sum(res$pc_loadings > 0) == length(res$pc_loadings)
  all_neg = sum(res$pc_loadings < 0) == length(res$pc_loadings)
  if(all_pos) {
    message('All PC loadings are positive')
  } else if(all_neg) {
    message('All PC loadings are negative')
    res$pc_loadings = - res$pc_loadings
    res$pc = - res$pc
  } else {
    message('PC loadings are not all positive or negative')
  }
  res
}