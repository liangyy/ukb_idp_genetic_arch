...
# load t1
t1_mat = read_parquet(paste0(second_dir, '/output/t1.scaled.all_covar.parquet'))
...
for(i in 1 : nrow(df_tag)) {
  ...
  if(doit | force_run) {
    ...
    if(isTRUE(save_df) | isTRUE(save_small)) {
      saveRDS(
        list(
          pc_loadings = data.frame(pc_loading = kk$res$pc_loadings, IDP = colnames(cort_all)), 
          pve = kk$res$pve
        ), 
...
if(doit | force_run) {
  ...
  if(isTRUE(save_df)) {
    write_parquet(df_out, paste0(outdir, '/fourth_round.t1_w_pc.parquet'))
  }
  ...
  if(isTRUE(save_df)) {
    write_parquet(t1_mat, paste0(outdir, '/fourth_round.t1_no_pc.parquet'))
  }
}