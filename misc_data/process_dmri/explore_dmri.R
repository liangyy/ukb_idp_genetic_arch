...
t1_meta_rds = '../download_some_matching_files/annot_dmri_idps.rds'
...
df_v = df_v %>% left_join(t1_meta %>% mutate(idp = paste0('IDP-', FieldID)) %>% select(idp, type, position, measure, lr), by = 'idp')
...
df_corr = corr %>% reshape2::melt() %>% 
  inner_join(t1_meta %>% mutate(idp = paste0('IDP-', FieldID)) %>% select(idp, type, position, measure, lr), by = c('Var1' = 'idp')) %>%
  inner_join(t1_meta %>% mutate(idp = paste0('IDP-', FieldID)) %>% select(idp, type, position, measure, lr), by = c('Var2' = 'idp'), suffix = c('.1', '.2'))
