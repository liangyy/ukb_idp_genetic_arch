...
myplot_magic2 = function(df) {
  tmp = df[!duplicated(df$Var1), ]
  tmp = tmp %>% arrange(position.1, lr.1, measure.1, type.1) %>% pull(Var1)
  df$Var1 = factor(df$Var1, levels = tmp)
  df$Var2 = factor(df$Var2, levels = tmp)
  mytmp = df #
  mytmp$Var1 = factor(mytmp$Var1, levels = tmp)
  # mytmp_g = mytmp[!duplicated(mytmp$Var1), ] %>% mutate(order = order(Var1)) %>% group_by(position.1) %>% summarize(pos = mean(order))
  mytmp %>% ggplot() +
    theme_bw() + 
    geom_raster(aes(x = Var1, y = Var2, fill = value)) + scale_fill_gradient2(low = 'blue', high = 'red', mid = 'white') +
    
    # annotate(geom = "text", x = mytmp_g$pos, y = -20, label = mytmp_g$position.1, size = 4) + 
...
p1 = myplot_magic2(df_corr %>% filter(type.1 == 'mean', type.2 == 'mean') )
p2 = myplot_magic2(df_corr %>% filter(type.1 == 'weighted_mean_in_tract', type.2 == 'weighted_mean_in_tract') )
# p3 = myplot_magic2(df_corr %>% filter(type.1 == 'weighted_mean_in_tract', type.2 == 'mean') )
ggsave('dmri_residual_mean_corr_pve0.5.png', p1)
ggsave('dmri_residual_weighted_mean_corr_pve0.5.png', p2)
# ggsave('dmri_residual_cross_corr_pve0.5.png', p3)

res2 = split_by_pca(mat, npc = npc)
mat_res2 = res2$residual
pc_all2 = res2$pc

corr2 = cor(mat_res2)
df_corr2 = corr2 %>% reshape2::melt() %>% 
  inner_join(df_annot %>% mutate(idp = paste0('IDP-', FieldID)) %>% select(idp, type, position, measure, lr), by = c('Var1' = 'idp')) %>%
  inner_join(df_annot %>% mutate(idp = paste0('IDP-', FieldID)) %>% select(idp, type, position, measure, lr), by = c('Var2' = 'idp'), suffix = c('.1', '.2'))
df_corr2 = df_corr2 %>% mutate(
  label1 = paste0(Var1, '\n', measure.1),
  label2 = paste0(Var2, '\n', measure.2)
)
p11 = myplot_magic2(df_corr2 %>% filter(type.1 == 'mean', type.2 == 'mean') )
p22 = myplot_magic2(df_corr2 %>% filter(type.1 == 'weighted_mean_in_tract', type.2 == 'weighted_mean_in_tract') )
# p3 = myplot_magic2(df_corr %>% filter(type.1 == 'weighted_mean_in_tract', type.2 == 'mean') )
ggsave('dmri_residual_mean_corr_npc50.png', p11)
ggsave('dmri_residual_weighted_mean_corr_npc50.png', p22)
# ggsave('dmri_residual_cross_corr_npc50.png', p3)
# 
