...
for(m in names(models)) {
  for(t in names(idps)) {
    fn1 = paste0('~/Desktop/tmp/ukb_idp/data/imagexcan_round_2.linear.', idps[[t]], '_', models[[m]], '.csv')
    tmp1 = read.csv(fn1)
    fn2 = paste0('~/Desktop/tmp/ukb_idp/data/imagexcan_round_2.susie.', idps[[t]], '_', models[[m]], '.csv')
    tmp2 = read.csv(fn2)
