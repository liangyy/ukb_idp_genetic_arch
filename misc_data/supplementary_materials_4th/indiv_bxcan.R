...
for(m in names(models)) {
  for(t in names(idps)) {
    fn1 = paste0('~/Desktop/tmp/ukb_idp/data/imagexcan_linear.fourth_round_residual.', idps[[t]], '.', models[[m]], '.csv')
    tmp1 = read.csv(fn1)
...
if(isTRUE(plot_i_bxcan)) {
  ...
  tmp = inner_join(
    df %>% filter(model == 'ridge') %>% select(IDP, phenotype, model, idp_type, zscore),
    df %>% filter(model == 'elastic net') %>% select(IDP, phenotype, model, idp_type, zscore),
    by = c('IDP', 'phenotype', 'idp_type'),
    suffix = c('.ridge', '.en')
  )
...
if(isTRUE(do_sbxcan_compare)) {
  ...
  load_sbxcan = function(folder, trait_list) {
    ...
    for(t in trait_list) {
      for(i in names(idp_type)) {
        # for(m in names(models)) {
        df1[[length(df1) + 1]] = read.csv(paste0(folder, '/', idp_type[[i]], '_x_', t, '_x_simagexcan.csv'), header = T) %>% mutate(idp_type = i, phenotype = t)
        # }
      }
    }
  ...
  df_sb = load_sbxcan('~/Desktop/tmp/ukb_idp/simagexcan/results_gtex_gwas_4th_en_residual/', phenos) 
  df_sb = df_sb %>% mutate(zscore = p2z(pval, bhat), model = 'elastic net')
  df_sb2 = load_sbxcan('~/Desktop/tmp/ukb_idp/simagexcan/results_gtex_gwas_4th_ridge_residual/', phenos) 
  df_sb2 = df_sb2 %>% mutate(zscore = p2z(pval, bhat), model = 'ridge')
  df_sb = inner_join(rbind(df_sb, df_sb2), data.frame(pheno = c('height', 'bmi'), phenotype = c('UKB_50_Standing_height', 'UKB_21001_Body_mass_index_BMI')), by = c('phenotype'))
  df_in = df %>% filter(phenotype %in% c('height', 'bmi'))
  df_in = df_in %>% mutate(zscore = p2z(pval, bhat))
  df_both = inner_join(
    df_sb %>% select(IDP, bhat, zscore, model, idp_type, pheno), 
