...
outdir = 'misc_summary_bxcan'
dir.create(outdir)

kk = readRDS('indiv_bxcan/dataframe_full.indiv_bxcan.rds') %>% filter(model == 'ridge')
kk2 = readRDS('s_bxcan/dataframe_full.sbxcan.rds') %>% filter(model == 'ridge')
kk3 = readRDS('s_bxcan/dataframe_full.gencor.rds')
df_all = rbind(
  kk %>% select(IDP, phenotype, bhat, pval, zscore, idp_type, model, idp_type) %>% mutate(source = 'indiv_bxcan'),
  kk2 %>% select(IDP, phenotype, bhat, pval, zscore, idp_type, model, source)
) 
...
df_scz = kk2 = readRDS('s_bxcan/dataframe_full.sbxcan.rds') %>% filter(model == 'ridge') %>% filter(phenotype == 'SCZ_PGC_2020')
...
p = df_ridge %>% mutate(rank_z = rank(abs(zscore))) %>% 
...
p = df_ridge %>% mutate(is_pc = annot_pc(IDP)) %>% ggplot() +
