...
outdir = 'misc_summary_bxcan_permz'
dir.create(outdir)
...
kk2 = readRDS('s_bxcan_permz/dataframe_full.sbxcan_permz.rds') %>% filter(model == 'ridge')
...
df_all = rbind(
  ...
  kk2 %>% select(IDP, phenotype, bhat, pval_adj_perm_null, z_adj_perm_null, modality, model, source) %>%
    rename(pval = pval_adj_perm_null, zscore = z_adj_perm_null, idp_type = modality)
) 
