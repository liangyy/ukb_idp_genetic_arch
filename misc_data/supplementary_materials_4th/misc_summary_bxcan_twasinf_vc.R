...
outdir = 'misc_summary_bxcan_twasinf_vc'
dir.create(outdir)
...
kk2 = readRDS('s_bxcan_twasinf_vc/dataframe_full.sbxcan_twasinf.rds') %>% filter(model == 'ridge')
...
if(TRUE) {
  ...
  df_scz = kk2 = readRDS('s_bxcan_twasinf_vc/dataframe_full.sbxcan_twasinf.rds') %>% filter(model == 'ridge') %>% filter(phenotype == 'SCZ_PGC_2020') %>% select(-pval) %>% rename(idp_type = modality, pval = pval_corrected)
  ...
  df_scz_sub = df_scz_sub %>% mutate(p_adj = pval / rank(pval, ties.method = 'max') * n()) 
  print(
  df_scz_sub %>% filter(p_adj < alpha * 2) %>% nrow())
  
  top_10s <- df_scz_sub %>% arrange(pval) %>% head(10)
  qq2 <- read.csv('~/Desktop/rerun_brainxcan/output/SCZ.sbrainxcan.csv', )
  qq2 <- qq2[qq2$IDP %in% df_scz_sub$IDP, ]
  qq2 %>% arrange(pval_adj_vc) %>% head(10) %>% filter(IDP %in% top_10s$IDP)
  top_10s$p_adj
  mr <- list()
  for(idp in top_10s$IDP) {
    ff <- Sys.glob(glue::glue('~/Desktop/rerun_brainxcan/output/SCZ_files/MR_results/*{idp}.*sumstats.tsv'))
    if(!file.exists(ff[1])) {
      message(idp)
      next
    }
    qq <- read.table(ff, header = TRUE, sep = '\t')
    mr[[length(mr) + 1]] <- qq %>% filter(method == 'ACAT meta-analysis') %>% mutate(IDP = idp)
  }
  mr <- do.call(rbind, mr)
  mr %>% filter(direction == 'IDP -> Phenotype', pval < 0.05)
  mr %>% filter(direction == 'Phenotype -> IDP', pval < 0.05)
  # side analysis 
  common_pairs = list(
