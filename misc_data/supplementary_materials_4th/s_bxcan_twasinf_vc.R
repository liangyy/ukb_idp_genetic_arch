...
df <- df %>% left_join(df_gwas %>% select(phenotype_id, sample_size, h2), by = c('phenotype' = 'phenotype_id')) %>%
  ...
  mutate(corrected_zscore = sign(z_brainxcan) * sqrt(z_brainxcan ^ 2 / (1 + phi * sample_size * h2))) %>%
  mutate(corrected_pvalue = exp(pchisq(corrected_zscore ^ 2, df= 1,
...
if(isTRUE(common_pheno_s)) {
  ...
  for(cc in names(common_pairs)) {
    ...
    plist[[length(plist) + 1]] = tmp %>% filter(model == 'ridge') %>% ggplot() + geom_point(aes(x = x, y = y), alpha = 0.5) + 
      ...
      xlab(paste0('S-BrainXcan z-score from\n', 'GWAS ', namex)) + 
      ylab(paste0('S-BrainXcan z-score from\n', 'GWAS ', namey)) + th +
      ggtitle(cc) +
      theme(plot.margin = margin(www, www, www, www, unit = "pt")) +
...
if(isTRUE(gen_cor_s)) {
  ...
  if(isTRUE(gen_cor_qq_s)) {
    ...
    p = tmp %>% ungroup() %>% mutate(model = factor_methods(model)) %>% 
      ...
      xlab(expression(paste(-log[10], p[expected]))) + 
      ylab(expression(paste(-log[10], p[observed]))) +
      theme(legend.position = c(0.65, 0.72), legend.title = element_blank(), legend.background = element_blank())
