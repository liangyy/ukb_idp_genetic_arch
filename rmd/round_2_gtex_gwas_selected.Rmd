...
for(dd in idps) {
  ...
  for(i in 1 : nrow(sub)) {
    tmp = readRDS(paste0('~/Desktop/tmp/ukb_idp/mr_gtex_gwas_2nd/MR_local.gtex_gwas_2nd_', dd, '.', sub$IDP[i], '_x_', sub$phenotype[i], '.rds'))
...
```{r, fig.width=15, fig.height=5}
df_tmp = df_mr %>% filter(is_pass == T) %>% select(pheno, IDP) %>% distinct()
