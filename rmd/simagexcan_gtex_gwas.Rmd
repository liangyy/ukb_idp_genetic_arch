...
```{r}
dd = read.table('../misc_data/simagexcan_gtex_gwas.t1.signif.tsv', header = T)
dd %>% select(pheno, pheno_code) %>% distinct()
paste0(unique(dd$pheno), collapse = "', '")
```
...
for(i in 1 : nrow(df_signif)) {
  tmp = readRDS(paste0('~/Desktop/tmp/ukb_idp/mr/MR.gtex_gwas_t1.', df_signif$idp[i], '_x_', df_signif$pheno[i], '.rds'))
