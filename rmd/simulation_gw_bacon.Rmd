...
```{r}
library(bacon)
get_bacon_p <- function(name, zscore) {
  res <- tryCatch( {
    tmp <- bacon(zscore)
    data.frame(name = name, pval_bacon = pval(tmp)[, 1]) }, 
    error = function(cond) {
      return(data.frame(name = name, pval_bacon = NA))
    }
  )
  return(res)
}
df_ideal_b <- df_ideal %>% 
  mutate(z = p2z(pmax(pval, .Machine$double.xmin), bhat)) %>%
  group_by(h2, pve, type, rand) %>%
  do(get_bacon_p(.$x, .$z))
df_b <- df %>% 
  mutate(z = p2z(pmax(pval, .Machine$double.xmin), eff)) %>%
  group_by(method, h2, pve, type, rand) %>%
  do(get_bacon_p(.$m, .$z))
df_ideal <- df_ideal %>% left_join(df_ideal_b, by = c('h2', 'pve', 'type', 'rand', 'x' = 'name'))
df <- df %>% left_join(df_b, by = c('method', 'h2', 'pve', 'type', 'rand', 'm' = 'name'))
```
...
df_all <- rbind(
  df_ideal %>% 
    ...
    rename(eff = bhat, m = x) %>%
    select(m, eff, pval, pval_adj_permz, pval_bacon, method, h2, pve, type, rand) %>% 
...
df_all$pval_bacon[df_all$pval_bacon < min_p] <- min_p 
df_all$pval_bacon[df_all$pval_bacon > 1 - min_p] <- 1 - min_p
df_all <- df_all %>% 
  mutate(h2_y = pve * h2) %>% 
  mutate(z = get_z(pval, eff)) %>% 
  mutate(z_adj_permz = get_z(pval_adj_permz, eff)) %>% 
  mutate(z_adj_trunc = get_z(pval_adj_trunc, eff)) %>% 
  mutate(z_bacon = get_z(pval_bacon, eff))
```
...
df_all %>% 
  filter(type == 'null') %>%
  group_by(h2_y, method) %>% mutate(p_exp = get_p_exp(pval_bacon)) %>%
  ungroup() %>%
  ggplot() + 
  geom_point(aes(x = -log10(p_exp), y = -log10(pval_bacon), color = method)) + 
  geom_abline(intercept = 0, slope = 1) +
  facet_wrap(~h2_y, labeller = label_both) + th2 +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = .5))
```
...
df_all %>% 
  filter(type == 'null') %>%
  ggplot() + 
  geom_point(aes(x = -log10(pval), y = -log10(pval_bacon), color = method)) + 
  geom_abline(intercept = 0, slope = 1) +
  facet_wrap(~h2_y, labeller = label_both) + th2 +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = .5))
```
...
df_all %>% 
  ...
  summarize(varz = var(z), varz_permz = var(z_adj_permz), varz_trunc = var(z_adj_trunc), varz_bacon = var(z_bacon)) %>%
  ungroup() %>%
...
df_all %>% 
  filter(type == 'alt') %>%
  group_by(pve, h2, method) %>% mutate(p_exp = get_p_exp(pval_bacon)) %>%
  ungroup() %>%
  ggplot() + 
  geom_point(aes(x = -log10(p_exp), y = -log10(pval_bacon), color = method)) + 
  geom_abline(intercept = 0, slope = 1) +
...
df_all %>% 
  filter(type == 'alt') %>%
  ggplot() + 
  geom_point(aes(x = -log10(pval_adj_permz), y = -log10(pval_bacon), color = method)) + 
  geom_abline(intercept = 0, slope = 1) +
  facet_grid(h2~pve, labeller = label_both) + th2 +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = .5))
```
...
df_all %>% 
  filter(type == 'alt') %>%
  ggplot() + 
  geom_point(aes(x = -log10(pval), y = -log10(pval_adj_permz), color = method)) + 
  geom_abline(intercept = 0, slope = 1) +
  facet_grid(h2~pve, labeller = label_both) + th2 +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = .5))
```
