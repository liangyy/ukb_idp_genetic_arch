# args1: nametag
nametag=$1
outdir=/scratch/t.cri.yliang/ukb_idp/gw_elastic_net_dmri_4th/$nametag
...
outdir2=/scratch/t.cri.yliang/ukb_idp/gw_elastic_net_dmri_4th
if [[ -f $tmplist ]]
then
  rm $tmplist
fi
...
do
  echo "$outdir/$l.gw_lasso.performance.tsv" >> $tmplist
done
...
python /gpfs/data/im-lab/nas40t2/yanyul/GitHub/ukb_idp_genetic_arch/methods/gw_lasso/post_cv_perf.py \
  --model_list $tmplist \
  --rename_yaml $rename_yaml \
  --output $outdir2/$nametag.gw_elastic_net_beta.perf.tsv.gz
