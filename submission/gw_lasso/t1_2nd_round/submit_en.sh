# args1: nametag
nametag=$1
phenopre=/gpfs/data/im-lab/nas40t2/yanyul/GitHub/ukb_idp_genetic_arch/submission/gw_lasso/t1_2nd/list_misc/$nametag/pheno_list
batchsize=2
...
do
  ...
  then
    ...
    if [[ $tmp = 1 ]]
    then
      echo qsub -v BATCH=$i -N gw_en_$i run_en.qsub
    fi
  else
    # :
    echo qsub -v BATCH=$i -N gw_en_$i run_en.qsub
